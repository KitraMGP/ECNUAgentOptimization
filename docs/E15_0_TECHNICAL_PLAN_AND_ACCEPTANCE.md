...
| **E15.6** | 收口：完整验证 + 复现命令 + 提交 | `E15_FINAL_REPORT.md` | **根全量 pytest 全绿**（`cd benchmark && uv run pytest -q`；413 为截至 2026-08-08 阶段 0 的实测数，**验收以"全量绿"为准、不验固定数**，后续新增测试以当时实测为准）+ llama.cpp 相关测试全绿；两仓库 clean；**4B 门禁（M6）**：B1 lossless 若声明 PASS 必须已有 4B greedy paired 证据；12.22 唯一状态 | 按 12.22 判定 | 外部阻塞（12.15） | 按 12.22 REJECT 清单 |
