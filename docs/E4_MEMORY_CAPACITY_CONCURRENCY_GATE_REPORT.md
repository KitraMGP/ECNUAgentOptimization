...
- **真实 OOM 边界 >96%**：E3.1 已验证超池场景（A 5109 + B 5094 > 8192）
  purge/淘汰成功恢复（无 400）——A1/A4 淘汰机制在池满时不 OOM；
...
1. **并发承载**：unified 池共享支持 ≥8 并发 session（0 失败、KV 利用率
   48%）——容量随池共享扩展；
2. **KV cache 使用**：利用率随并发/轮数增长（23%→96%），容量指标可精确
   观测（/metrics/kv）；
3. **OOM 边界**：>96%（6×12 未超池）；超池场景（E3.1）purge/淘汰恢复
   ——**A1/A4 淘汰机制保证池满不 OOM**；
