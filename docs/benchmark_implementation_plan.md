...
> **状态：已由实际执行取代（superseded）**。本计划是 E0 前的三阶段工程规划
> （纯 Python 增强 → llama.cpp 可观测性 → 优化验证）。实际执行中阶段编号与
> 内容已演进为 E0-E4（基础设施 / 可观测性 / 生命周期候选 / 价值感知回收与
> 诊断 / 内存容量验证），各阶段结论以 `docs/E*_*.md` 为准（索引见根 README）。
> 本文件保留作为设计决策记录。
