...
| `parity_ok` | bool | **是** | **parity 阶段完成状态总规则（v35）**：parity 全部桶完成且偏差=0 → `true`；全部完成但偏差≠0 → `false`；未完整完成 → `null`（不按 preflight_reason 硬编码） | `null`（未完整完成） |
...
| `token_count_method` | string | **是** | **总规则（v35）**：完整执行 apply-template+tokenize 时 = `"apply-template+tokenize"`（无论 parity pass/mismatch）；未完整建立 → `null` | `null`（未完整建立） |
