...
```json
{
  "meta": {"workload": "fanout", "design_ref": "M0_BRANCH_MEMORY_BASELINE_DESIGN.md",
           ...
           "decision_validation": {"sessions": [  # v13（M2/G-M0-1 证据）：两次独立 server 启动；formal 强制 2 sessions×≥10，preflight 允许 null/partial
               ...
               # partial 示例（v18，preflight 合法；等式与 error_summary 约束同时成立）：{"sessions": [{"server_start": 1, "requests": 5, "valid": 4,
               #   "invalid": 0, "error_count": 1, "finish_reasons": {"stop": 4, "length": 1}, "output_hashes": [...],
               #   "error_summary": [{"code": "validation_incomplete", "stage": "validation", "count": 1}]}],
               #   "total_valid_rate": 0.8}   # requests=5=4+0+1（等式成立）；sum(error_summary[].count)=1=error_count（约束成立）；sum(valid)/sum(requests)=4/5；层级无歧义
