...
> 状态：**完成（v69 两轮正式矩阵）——run6/run7 均 verdict=PASS、120/120 OK、G-M0-6 复现 PASS；两层结论：A) benchmark/归因门禁 PASS；B) on 机制 4B hybrid 被 capability gate 拒绝（shared_cells=0）→ 优化收益 NOT_APPLICABLE/NO_GAIN。run5 保持 INVALID_INTERMEDIATE。最终结论文档：`docs/M0_FINAL_RESULT.md`**。
> 日期：2026-08-09 ｜ 对应路线：M0（阶段 0 通过后第一条后续路线）｜ 实施建议：**GO**（见 §8）。
**v69（两轮完整 24-unit 4B formal 矩阵 run6/run7——wrapper 启用后的正式结论，2026-08-09）**：（1）**run6/run7 均 `verdict=PASS`**：120/120 reps OK、0 ERROR（vs run5 36/120 connection_error——transient retry 是 run5 高失败率根因修复，非掩盖）、24/24 units、12 groups、matrix_complete=true、canonical validator OK、notes=0；gates G-M0-1/2/3a/4/5/7 PASS、G-M0-3b 恒 N/A；（2）**G-M0-6 复现性**（**归档后跨轮计算，非单 run 机器 gate**；单轮 gates N/A）：latency 中位数 467.0 vs 451.7 ms（dev 3.29%）、ttft 261.3 vs 267.0 ms（dev 2.15%）、decision_fallback 全 False 一致（阈值 ≤10%）；**分支归属一致量化**：跨轮 120 对 + off-on 严格配对各 60 对 `decision_fallback` mismatch 均 0；同 unit+rep 120 对 status 全一致、latency median +0.36%、peak_gpu/kv 跨轮 median 一致；（3）**严格 paired off/on**（同 run、同 fanout/bucket/ctk/ctv/rep、两边均 OK——120/120 OK → 每 run 60 对）：run6 all latency +0.24%/ttft -0.07%、run7 +0.62%/+0.99%——**run7 fanout=f8 同时 latency +6.88%/ttft +10.27% 为 n=10 小样本（parallel 10 高并发）噪音例外**，其余维度 ±≤3%、两轮方向不一致 → 纯噪音、无系统性 on 收益；（4）**两层结论**：A) M0 benchmark/归因门禁 **PASS**（workload/生命周期/观测/归因/复现性完备）；B) on 机制 4B hybrid 被 capability gate 拒绝——**主证据 on g6-g11 capability rejection 日志**（key-lines `E8-C1: capability rejected: memory implementation does not support cross-slot prefix metadata sharing`，共享未建立），**两轮 24 个 group baseline /metrics/kv 快照 shared_cells 全 0 仅一致性佐证**（不称全部周期样本）→ **优化收益 NOT_APPLICABLE/NO_GAIN，benchmark PASS 不得称共享优化 PASS**；（5）资源峰值：组级 baseline gpu_used_mb 3026–3414（mean 3202，两轮逐 group 相同）、rep peak_gpu max 3424 MiB、kv peak cells max 3192/4096（无 OOM）；（6）归档 `benchmark/baseline/`：run6/run7 主 JSON + evidence + 15-tag key-lines + run6v7 paired JSON（`docs/M0_FINAL_RESULT.md` 清单）；run5 保持 INVALID_INTERMEDIATE；（7）无代码改动（纯运行+归档）——全量 pytest 沿用 v68 HEAD **758 passed**；llama.cpp 零改动。详细数据见 `docs/M0_FINAL_RESULT.md`。
**v68（v67 复审 2 Medium 修复——显式 crashed 信号 + 文本 fallback 收紧，2026-08-09）**：（1）**`_run_decision_session` 返回显式 `(recs, crashed)` 信号**——`run_decision_validation` 据 crashed **无条件停止后续 session**，**不再用 `len(recs)<target` 推断**（v68 边界修复：第 target 个请求 ServerCrash 时 `len(recs)==target` 旧推断不停止，会错误启动下一个 server）；崩溃请求仍计入 requests/error_count/error_summary code=server_crash；**覆盖第 1/5/10 请求崩溃边界**（`test_m0_v67_review.py` 参数化 crash_at 0/4/9 → requests 1/5/10，均当前请求计 error 且不启动后续 server）+ 第二 session 崩溃 + 无崩溃反 e2e（3→5 例）；（2）**文本 fallback 收紧并单一 helper**——新增模块级 `_text_transient`（`_transient_class` 与 `classify_error` 共用，模式完全一致）：timeout 匹配 "timed out"/"timeout"；connection 匹配 `\bconnection\b`（connection refused/reset/error/lost/closed 等明确语义）或 failed to connect / unable to connect / cannot connect；**裸 "connect" 不再匹配**（disconnect/reconnect/connectivity 等内部消息不误判）；补误匹配反例（Disconnected from server/disconnected/Reconnecting/connectivity 等）+ 明确模式正例 + classify_error 共享 helper 一致性测试（v66 21→25 例）；（3）全量 pytest **752 → 758**（v66 +4、v67 +2）；未跑完整 GPU 矩阵、llama.cpp 零改动。
