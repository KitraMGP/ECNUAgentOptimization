...
```json
{
  ...
  #   - **每 unit 固定 formal reps = 5（v43）**；每 rep 项必含：unit 身份字段 + **`rep_index`（0..reps-1）** + rep 级 `status`/`decision_fallback`/metrics
  #   - **validator（v43）**：`(unit_id, rep_index)` 全局唯一（unit_id 允许在 5 个 rep 中重复）；每个已执行 unit 的 rep_index 集合必须为 `{0..4}`（ERROR/中断 unit 可为前缀并使 matrix_complete=false）；24 完整性按 replicates 中 unit_id 去重集合验证
...
㉗ **多 cache profile 编排单测（v43）**：per-unit 字段缺失/错误 server_group_id/profile → SCHEMA_INVALID；**`(unit_id, rep_index)` 全局唯一——unit_id 允许在 5 个 rep 中重复（v43，删除「重复 unit_id 即非法」旧断言）；每个已执行 unit 的 rep_index 集合必须 {0..4}（ERROR/中断 unit 可为前缀）；expected 24 unit_id 去重集合唯一完整（扣除 preflight_rejections 后 executed 匹配）**；**12 server group 生命周期（cache profile × fanout × control = 2×3×2，每组 parallel=fanout+2、独立启动/回基线/停止）；profile/fanout/control 重启；崩溃 group → 该 group 不完整且 matrix_complete=false**；**第 2+ group 启动失败或任一 group 运行中崩溃 → formal + FORMAL_INCOMPLETE + 保留已完成 group/unit + 停止剩余（v42）**；**group 对象逐项覆盖（v42）**：status∈{COMPLETED,ERROR}、start/stop RFC3339 UTC、baseline 内联六字段（server_version/gpu_used_mb/rss_mb/rs_buffer_mb/kv_buffer_mb/metrics_kv_snapshot）、ERROR 时 error_type 必填（12 码子集）+error_stage=formal、COMPLETED 时 error 字段禁止；**rejections 覆盖（v42）**：unit 级 {unit_id,budget,threshold,reason:budget_rejected}、某桶超限展开 4 unit_id、unit_id 唯一子集、executed=24−len(rejections) 仅 formal 且 matrix_complete=true、所有 preflight executed=0；**rep 粒度覆盖（v43）**：replicates 每项=正式 rep、warmup_count=2 必填整数、rep 级 status/decision_fallback/metrics、每 unit 固定 5 reps、rep_index∈0..4；**FORMAL_INCOMPLETE 子集覆盖（v43）**：modes 只含已启动 groups（失败 group 以 ERROR 出现、未启动不出现）、subset unit 且 len(去重 unit_id)==executed_units（仅完整完成 5 reps 的 unit 计 executed、部分 rep 不计）；**per-unit 单 bucket 覆盖（v42）**：bucket 枚举、prefix_len/branch_len int 且同桶 4 unit 一致、unit_id 中 bucket==该字段。
