...
```json
{
  ...
  #   - **validator（v45）**：`(unit_id, rep_index)` 全局唯一（unit_id 允许在 5 个 rep 中重复）；每个已执行 unit 的 rep_index 集合 ⊆ `{0..FORMAL_REPS−1}`；**complete = rep_index 完整 `{0..FORMAL_REPS−1}` 的 unit（不要求 status!=ERROR，v45）；executed_units = len(complete_unit_ids)；observed ⊆ EXPECTED_UNIT_IDS − rejections；matrix_complete=true → observed==complete==EXPECTED−rejections，false → 允许任意子集（非"前缀"）；删除 v43 的 len(去重 unit_id)==executed_units 旧等式（v45）**
