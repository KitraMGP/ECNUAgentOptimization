...
> 修订：2026-08-09 v2（矩阵/预算/层数/架构/接口合同/回收观测）；v3（精确 token 计数、矩阵手算修正、driver 合同、门禁语义）；v4（driver `_retry` 位置参数兼容、thinking 字段生效实证、G-M0-1 与矩阵完全分离、RS 日志行号、CLI 临时目录）；v5（G-M0-1 口径统一、可提交证据、token parity 门禁、verdict 规则）；v6（INVALID 谓词 OR 语义、verdict/gates 值域映射、add_special 归因、parity 校准协议）；v7（gate/verdict 分层、parity 前置、schema 完整化、聚合定稿）；v8（preflight/schema 闭环）；v9（状态机/schema 终审）；v10（验证/聚合路径收口）；v11（跨章节同步定稿）；v12（运行边界合同）；v13（实现阻断收口）；v14（validator/测试合同终审）；v15（落盘/测试盲区终审）；v16（结构化错误/schema/测试残留终审）；v17（最后 Medium/Low 文档残留）；v18（终审阻断与规范残留）；v19（示例不变量终审）；v20（决策统计不变量终审）；v21（测试/schema 残留终审）；v22（GO 后终审剩余 Medium/Low）；**v23（验证崩溃边界与测试缺口——§3.5 崩溃边界统一（两次验证 session 均达 requests≥10 且每请求有记录（含最后 in-flight 崩溃但完整）→ 证据完整继续 formal、不要求 server health 正常；启动失败/崩溃后 requests<10/证据无法落盘 → 5b PREFLIGHT_INFRA+partial，删除"崩溃即 5b"相反旧表述）、测试⑬拆 ⑬a（session 完整计入 formal 2 sessions）与 ⑬b（session 不完整 → 5b partial + 断言崩溃请求计入 requests/error_count + 全部等式成立）、测试㉖ 补容量字段正反例（planned_units=24/executed_units=0/matrix_complete=false 正例；任一违反 → SCHEMA_INVALID）、planned_units 派生值定义（len(理论合法组合6)×len(cache_types2)×len(controls2)=24，validator 按矩阵合同计算期望值，注释改"全量计划单元数，与执行进度无关"）、§3.7 崩溃后回收（finally 路径 wait/reap 进程对象/子进程/端口，无 zombie/端口残留，测试⑰ 补崩溃路径断言））**，见各节。
...
5. G-M0-1 专用稳定性验证：2 次**独立 server 启动** × 每次 ≥10 请求（总数 ≥20）
   ...
   —— **验证阶段失败路径（v12，任务 5）**：
      a) 验证中普通请求 ERROR/invalid：**如实记入 meta.decision_validation**（含结构化 error_summary），G-M0-1=FAIL；
         ...
         **边界（v23 统一）**：**两次验证 session 均达到目标 requests≥10 且每个请求都有 OK/INVALID_DECISION/ERROR 记录（包括最后 in-flight 崩溃但完整）→ 证据完整、继续 formal，不要求 server health 正常**（崩溃本身不影响证据完整性）；**启动失败、崩溃后 session requests<10、或证据无法落盘 → 走 5b `PREFLIGHT_INFRA`、`decision_validation` 为 partial**（不满足 formal 强制 2 sessions×≥10）；**删除"崩溃即 5b"的相反旧表述**（v23：仅上述三类触发 5b）；
...
- **生命周期（任务 8）**：
  ...
  - **崩溃后回收（v23，任务 5）**：验证 server 崩溃后，runner **必须在 `finally`/清理路径回收进程对象、子进程与端口**——即使进程已异常退出也执行 `wait()/reap`（`subprocess.Popen.wait()` 或 `os.waitpid`），确保**无 zombie 进程、无端口残留**（端口释放后可供下次启动复用）；测试⑰ 补崩溃路径断言（`pgrep` 无残留、端口可立即复用）。
...
```json
{
  "meta": {"workload": "fanout", "design_ref": "M0_BRANCH_MEMORY_BASELINE_DESIGN.md",
           ...
           "planned_units": 24,                   # v23：全量计划单元数，与执行进度无关；派生值 = len(理论合法组合 6) × len(cache_types 2) × len(controls 2) = 6×2×2 = 24（单一来源：矩阵合同 §4；validator 按同一公式计算期望值，矩阵变化时单点更新）
