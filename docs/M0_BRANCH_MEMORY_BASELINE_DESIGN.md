...
> 修订：2026-08-09 v2（矩阵/预算/层数/架构/接口合同/回收观测）；v3（精确 token 计数、矩阵手算修正、driver 合同、门禁语义）；v4（driver `_retry` 位置参数兼容、thinking 字段生效实证、G-M0-1 与矩阵完全分离、RS 日志行号、CLI 临时目录）；v5（G-M0-1 口径统一、可提交证据、token parity 门禁、verdict 规则）；v6（INVALID 谓词 OR 语义、verdict/gates 值域映射、add_special 归因、parity 校准协议）；v7（gate/verdict 分层、parity 前置、schema 完整化、聚合定稿）；v8（preflight/schema 闭环）；v9（状态机/schema 终审）；v10（验证/聚合路径收口）；v11（跨章节同步定稿）；v12（运行边界合同）；v13（实现阻断收口）；v14（validator/测试合同终审）；v15（落盘/测试盲区终审）；v16（结构化错误/schema/测试残留终审）；v17（最后 Medium/Low 文档残留）；v18（终审阻断与规范残留）；v19（示例不变量终审）；v20（决策统计不变量终审）；v21（测试/schema 残留终审）；v22（GO 后终审剩余 Medium/Low）；v23（验证崩溃边界与测试缺口）；v24（规则澄清收口）；v25（最后歧义收口）；v26（验证启动失败残留与清理测试）；v27（验证阶段最后歧义）；v28（evidence failure 路径与测试合同）；v29（SCHEMA_INVALID 落盘合同）；**v41（完整矩阵 CLI/per-unit schema 与 12 group 生命周期——CLI 正式入口一次恒跑完整 24-unit 矩阵（删除用户单值 --fanout/--prefix-len/--branch-len，维度由固定 MATRIX_PLAN 定义；可保留独立 debug/smoke 模式但不得产出 formal verdict，暂不设计；测试㉕ 同步未知参数）、top canonical meta 移除单值 parallel/fanout/prefix_len/branch_len（per-unit 承载；cache_profiles 保留；CANONICAL_META_KEYS 键数 26→22；表/示例/SCHEMA_INVALID/test⑯/AGENTS 同步；ctx_size 仍 top）、per-unit 权威表（unit_id/fanout/prefix_bucket/branch_bucket/ctk/ctv/prefix_len/branch_len/server_group_id；bucket 为 short/medium/long 枚举、len 为 apply-template+tokenize 精确整数 token 数；control 由父 modes.off/on 表达不在 unit 重复）、ID 格式固定可重建（unit_id={control}:{ctk}-{ctv}:f{fanout}:{bucket}、server_group_id={control}:{ctk}-{ctv}:f{fanout}；validator 校验格式/引用关系/全局唯一/24 预期集合（扣除 rejections））、formal 生命周期顺序 15 次进程（校准 1 + 验证 2 + formal 12 groups；每 group 独立启动/回基线/运行合法桶/停止；profile/fanout/control 由启动参数决定不在 server 内切换；§3.7/测试⑰ 同步）、group 启动失败/崩溃统一停止剩余矩阵（保留已完成 groups/units、当前 group 写 ERROR/group 失败记录、matrix_complete=false、executed_units=已完成 unit 数、FORMAL_INCOMPLETE 优先；仅首 group 失败才 executed_units=0/modes 可空；测试㉖/㉗ 覆盖首组与中途组失败）、parity_progress.errors 所有定义点统一四元组（parity_mismatch 强制 calibration+bucket+template；error_summary 无 template 三元组）、清理 ctk/ctv top 残留（示例/SCHEMA 段/测试⑯/nullable/AGENTS 改 cache_profiles；不保留 27 键旧说明）、§6 m0_schema.py 定义 SCHEMA_INVALID_FIXED 常量 + modes/per-unit validator + MATRIX_PLAN/EXPECTED_UNIT_IDS（单一来源）及引用、G-M0-4 按每个 server_group 的 parallel/ctk/ctv 分别对账启动日志 RS/KV 与公式、SCHEMA_INVALID 测试⑯ 从常量参数化不内联清单、phase 表预算全部拒限定 24 候选）**，见各节。
...
6. formal 矩阵（**v40：按 `cache profile × fanout × control` 分组启动，共 2×3×2 = 12 个独立 server group**；每组 `parallel = fanout + 2`，运行该 fanout 下合法长度桶（fanout 2 → 三桶 short/medium/long、fanout 4 → 两桶 short/medium、fanout 8 → 一桶 short）；**每组独立启动/回基线/停止，崩溃归属到该 group 并使 `matrix_complete=false`；不得用单 server 切换 cache type/parallel（v40）**；**group 启动失败/崩溃统一停止剩余矩阵（v41）**：保留已完成 groups/units，当前 group 写 ERROR 或 group 失败记录，`matrix_complete=false`、`executed_units` = 已完成 unit 数、**FORMAL_INCOMPLETE 优先**；**仅首 group 启动失败才 `executed_units=0` / modes 可空（v41）**；测试㉖/㉗ 覆盖首组与中途组失败；24 单元 = 12 group × 相应桶数，每单元 warmup 2 + reps 5）：
   —— **使用新的独立 server 启动**（与验证会话分离），先确认 attention 基线（used_cells==0）再开始（M2 防污染）；
   —— **formal server 启动失败（v22）** → 落盘 `phase=preflight`、规则 `PREFLIGHT_INFRA`、**`planned_units=24, executed_units=0`（v22：矩阵未启动）**、**`matrix_complete=false`**、**保留完整 decision_validation（验证证据不丢）**、`modes={}`、`gates={}`（§3.7 生命周期同步）；**启动失败 code 固定（v20）**：进程 spawn 失败/早退 → `server_crash`；进程存活但 health 超时 → `health_failed`；健康但必需端点缺失 → `endpoint_unavailable`
```
...
- **生命周期**：
  ...
  3. **formal 12 server groups（v41：顺序 15 次进程 = 校准 1 + 验证 2 + formal 12 groups）**：每个 formal group 独立启动/回基线（`used_cells==0`）/运行该 group 合法桶/停止；**profile/fanout/control 均由启动参数决定，不在 server 内切换（v41）**；
     - **formal server 启动失败（v22）**：health 探测失败/进程启动失败 → 落盘 `phase=preflight`、规则 `PREFLIGHT_INFRA`、**`planned_units=24, executed_units=0`（v22：矩阵未启动）**、**`matrix_complete=false`**、**保留完整 decision_validation**（验证证据不丢）、`modes={}`、`gates={}`，合法持久化（测试㉖）；**code 固定（v20）**：进程 spawn 失败/早退 → `server_crash`；进程存活但 health 超时 → `health_failed`；健康但必需端点（/metrics/kv、/apply-template、/tokenize）缺失 → `endpoint_unavailable`
...
| `preflight_rejections` | array | 否 | **仅记录 24 个候选（planned_units）经真实 apply-template+tokenize 精确复核后进一步超限的 unit（v36）；静态预筛排除（long×4 等）不写此处** | `[]` |
...
**nullable 白名单（v34）**：仅 `preflight_reason` / `parity_ok` / `parity_compensation`（**必填但允许 null 且当前恒 null，v34：从不可 null 示例删除**） / `token_count_method` / `decision_validation` 可 null；**不可 null 键启动前默认值（v34）**：`binary_version` 未知 → 固定 `"unknown"`；整数运行配置（ctx_size/parallel/fanout；**planned_units 由矩阵合同派生、非 CLI 输入，v36 从本列表移除**）**必须来自 CLI/矩阵合同，不得用魔法 0——配置本身缺失/非法属 CLI/config preflight 解析失败 → `EXIT_USAGE=64` + stderr `INVALID_CONFIGURATION`、不落盘实验结果（v35；不进入 SCHEMA_INVALID 包装）**；字符串配置（model_id/prefix_len/branch_len/ctk/ctv）来自配置、**不可用空串**；不可 null 默认示例：`parity_progress` 空结构 `{completed:[],errors:[]}`、`decision_validated=false`、`decision_fallback=false`、`preflight_rejections=[]`、`executed_units=0`、`matrix_complete=false`；**不得省略任何键**。
...
```json
{
  "meta": {"workload": "fanout", "design_ref": "M0_BRANCH_MEMORY_BASELINE_DESIGN.md",   # v31：canonical 固定键集（精确白名单，无第二套清单）
           ...
           "planned_units": 24,                   # v24：全量计划单元数，与执行进度无关；派生值 = len(理论合法组合 6) × len(cache_types 2 = ctk{q8_0,f16}) × len(controls 2 = 对照{A,B}) = 6×2×2 = 24；**理论合法组合数由 §4.2 预算函数单测锁定（v24）**，运行时精确 tokenizer rejection 只进 `preflight_rejections[]`、**不改变 planned_units**；validator 按同一矩阵合同公式计算期望值，矩阵变化时单点更新
           ...
           "parity_progress": {"completed": ["short-P", "short-B"], "errors": []},  # v18：{completed: list[str], errors: list[{code, stage?, bucket?}]}；preflight 可为空结构；PREFLIGHT_INFRA 部分完成时保存；formal 记录完整进度；不改变 verdict；**SCHEMA_INVALID 恒 {completed: [], errors: []}（v31，schema 错误只在 sidecar，不与 attribution code/stage 混用）**
  ...
  # v40：modes.{off,on}.replicates[] 每项为 per-unit 记录，必含精确 unit 字段：
  #   {unit_id, fanout, prefix_bucket, branch_bucket, ctk, ctv, prefix_len, branch_len, server_group_id, ...}
  #   control 由 modes.off/on 父级表达（选定唯一结构，per-unit 不重复 control，v40）；**per-unit 另含 `server_group_id`（引用该 group 启动日志/RS/KV 基线，v40）**；**G-M0-4 按实际 ctk/ctv 与 group 公式归因（v40）**；**validator 校验：24 个预期 unit_id 唯一完整（扣除 preflight_rejections 后 executed 匹配，v40）**。
**per-unit 权威表（v41）**：`unit_id`、`fanout`（2/4/8）、`prefix_bucket`（short/medium/long 枚举）、`branch_bucket`（short/medium/long 枚举）、`ctk`、`ctv`（q8_0/f16）、`prefix_len`、`branch_len`（**apply-template+tokenize 得到的精确整数 token 数，v41**）、`server_group_id`；control 由父 modes.off/on 表达不在 unit 重复。
**ID 格式固定并可重建（v41）**：`unit_id = {control}:{ctk}-{ctv}:f{fanout}:{bucket}`（如 `off:q8_0-q8_0:f4:medium`）、`server_group_id = {control}:{ctk}-{ctv}:f{fanout}`；**validator 校验格式、unit↔group 引用关系（unit 的 server_group_id 前缀 = 自身 control/ctk/ctv/fanout）、全局唯一与 24 预期集合（扣除 rejections）**
...
- **结构化错误码（v16）**：**不保存原始异常字符串/路径/URL/prompt**；`error_type`、`preflight_reason`、`error_summary`、`parity_progress.errors` 一律使用固定枚举 code（可选 `stage`/`bucket` 字段）：
  - **枚举（v29，12 个）**：`connection_error`、`timeout`、`http_4xx`、`http_5xx`、`malformed_response`、`server_crash`、`health_failed`、`endpoint_unavailable`、`parity_mismatch`、`budget_rejected`、`validation_incomplete`、**`schema_invalid`（v29：SCHEMA_INVALID 落盘专用）**；
  - **表示形式（v16）**：`error_summary` = **无错误时统一 `[]`，有错误时 `[{code, stage?, bucket?, count}]`（v39：不引入 template 维度；count 必需正整数）**；`parity_progress.errors` = `[{code, stage?, bucket?}]` 固定对象；`preflight_reason` = 单个固定 code 或 code 列表（无自由文本）；
  ...
  - **SCHEMA_INVALID 合法落盘包装（v30 定稿）**：
    - **主 envelope（v32）**：**不复制第二套 meta 清单——明确调用 `build_preflight_envelope`（v37：builder 名称统一，删除 canonical meta/envelope builder 别名）**，完整包含 §5.1 固定键集（`workload/design_ref/model_id/binary_version/ctx_size/parallel/fanout/prefix_len/branch_len/ctk/ctv/protocol/source_phase` 等），值从配置与受信运行上下文填充；**启动前未知字段按 canonical schema 指定 null 类型，不得省略**；**SCHEMA_INVALID 专有值（v39：**仅 canonical meta 权威表 SCHEMA_INVALID 列权威，本段纯引用不复制字段**；仅保留特有说明——notes 含 sidecar 相对路径、禁止递归 SCHEMA_INVALID、自检 70）**；**完整 result schema validator 必须接受该 envelope**（禁止递归 SCHEMA_INVALID）；
...
| `benchmark/runner/m0_schema.py` | 新增 | **canonical schema 模块（v39：引用 §5.1 当前 schema，不标注固定版本号）**——**`CANONICAL_META_KEYS` 常量（v36：meta 键集代码单一来源，builder/validator/测试⑯ 均引用；Markdown 表人工对应、不从 Markdown 解析；删除「14/27」含糊表述）**、`validate_result_envelope`（完整五键+phase+verdict+meta 固定键集）、`validate_schema_sidecar`（错误数组+独立枚举+RFC6901）、**parity_progress.errors 四元组 validator（{code,stage?,bucket?,template?}）与 error_summary 三元组 validator（{code,stage?,bucket?,count}，v40）**、**`SCHEMA_INVALID_FIXED` 常量（v41：SCHEMA_INVALID 固定值参数化单一来源）**、**`MATRIX_PLAN` / `EXPECTED_UNIT_IDS`（v41：矩阵计划与 24 预期 unit_id 集合单一来源，可放 fanout_prompts 或 schema 模块）**、**modes/per-unit validator（unit_id/server_group_id 格式与引用关系，v41）**、**build_preflight_envelope（v37：名称统一，删除 canonical envelope builder 别名；签名合同 v36）：`build_preflight_envelope(context, *, reason, source_phase, parity_ok, parity_progress, token_count_method, decision_validation, preflight_rejections)`——调用方传入 parity 状态，builder 不按 reason 暗中硬编码；schema_invalid/validation_incomplete 共用但传不同值，固定例外由调用方 + validator 检查**）、atomic writer（`.m0_fanout_<ts>.<kind>.tmp` + fsync + rename 顺序 + 陈旧 tmp 清理——匹配 `.m0_fanout_*.main.tmp` 与 `.m0_fanout_*.sidecar.tmp`，不留 envelope kind（v33））、exit constants（**`EXIT_USAGE=64`（v35）** / `EXIT_SCHEMA_INVALID=65` / `EX_SOFTWARE=70` / `EXIT_IO_WRITE_FAILED=74`——统一 64/65/70/74）；runner 调用；测试归属 `tests/test_m0_schema.py` |
...
| G-M0-4 内存归因（v4：启动日志精确值门禁） | **主门禁**：启动日志 `RS buffer size`（`llama-memory-recurrent.cpp:115`）精确值 vs 公式 `24×548864×4×parallel` 对账 **误差 ≤5%**；attention KV 同理（`llama_kv_cache` 分配日志 68/128 MiB vs `8×8×128×2×elem×4096`）；**按每个 server_group 的 parallel/ctk/ctv 分别对账启动日志 RS/KV 与公式（v41）**；**GPU used 差分只作总量 sanity**（阈值 ±10%，口径 = nvidia-smi `memory.used` 增量，探针观测 run-to-run 波动 <10%） |
