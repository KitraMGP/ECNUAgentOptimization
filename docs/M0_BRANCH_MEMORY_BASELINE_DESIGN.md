...
- `branch.py`（见 §2 详细分析）：固定 A/B 分支问题 + 固定"继续完善方案X"循环；
- `long_life.py`：多轮对话+工具+应用层截断，`est>budget` 丢早期消息，主指标 `state_retention_rate`（`:48-157`）。
- `branch_pressure.py`（E2.5，**不在注册列表**，需显式 import）：双分支 X/Y、长共享前缀占 ctx 40-70%、固定交替 8 请求 `SEQUENCE`、JSON evaluator（`:319-405`）。
...
**driver**（`benchmark/framework/driver.py`）：OpenAI SDK 封装走 `/v1/chat/completions`（`:58`）；`chat()` 请求前 `preprocessor.process(messages)`（默认 off）、`kv_probe` 前后快照、`extra_body` 固定 `enable_thinking=False`（`:62-142`）；返回行 `text/prompt_tokens/completion_tokens/total_tokens/cached_tokens/latency_ms/rss_mb/gpu_mb/timings`（`:89-99`）；`_extract_timings` 从 `timings` 属性 / `model_extra` / `__pydantic_extra__` 提取（`:160-180`）；400 超 ctx 重试丢早期 1/3 非 system 消息并保留真实 user query（`:144-158`）。**原生 `/completion` 未在 driver 封装**（e15_branch_concurrent 直接可用）。
...
**recurrent 内存**（`src/llama-memory-recurrent.cpp:99-126`）：每 recurrent 层分配 `cache_r_l{i}`/`cache_s_l{i}` 两个 tensor，**类型固定 F32**；`mem_size=max(1, n_seq_max)`（每 slot 一格，**与 n_ctx 无关**）；`n_rs_seq` 仅在 MTP/EAGLE3 等推测解码时非 0；启动打印 `RS buffer size` 与 `R/S (f32)` 分解（`:115, 123-126`）。
...
**`/completion` 与 OAI**：`server.cpp:242-246`（/completion、/completions、/v1/completions、/chat/completions、/v1/chat/completions 同一 `handle_completions_impl`）；响应 timings 填充 `server-context.cpp:568-576`（`cache_n/prompt_n/predicted_n`）；`id_slot` 在任务结构。**并发模型**：`server_queue::start_loop`（`server-queue.cpp:125-215`）主线程阻塞循环 = 队首 task → `update_slots()` 单次遍历所有 slots 完成 pre_decode/decode/sample（`server-context.cpp:3398+`）→ 即 cont-batching 是**单线程调度**，HTTP 线程独立（`server.cpp:504/519`）；并行只存在于 ggml 内部线程与多 slot 批处理。
...
**Qwen3.5-4B**：`llama_arch_is_hybrid` 含 `LLM_ARCH_QWEN35/QWEN35MOE`（`src/llama-arch.cpp:958-977`）；recurrent 层由 GGUF key `LLM_KV_ATTENTION_RECURRENT_LAYERS` 标记，缺失时 fallback `(i+1)%full_attn_interval!=0`（默认 interval=4 → 32 层中 8 层 recurrent）（`src/models/qwen35.cpp:21-31`）；4B 档 = `n_layer==32 && n_embd==2560`；memory 类型 `llama_memory_hybrid`（`llama-model.cpp:2256-2303`）；**`--cache-type-k/v` 只作用于 attention 部分**（`llama-model.cpp:2288-2296`），recurrent 固定 F32。

### 1.3 M0 探针实测（2026-08-09，RTX 4060 Laptop 8GB / driver 610.43.03）

方法：`build-cuda/bin/llama-server`（version 8568/f54930492，仅内存分配日志与版本无关）加载 `models/qwen3-5-4B-Q4_K_M.gguf`，`-ngl 99 --ctx-size 4096 --kv-unified`，轮询 `/health` 就绪后抓 nvidia-smi 与启动日志，立即 SIGTERM（**零推理请求**）。日志留于忽略目录 `llama.cpp/tmp/m0_probe2_*.log`、`m0_metrics_probe.log`（不入库）。
...
**关键量化事实**：
1. **attention KV（unified）与分支数无关**：ctx 4096 时 q8_0=68 MiB、f16=128 MiB 固定池——"KV 预分配固定"在 hybrid 上同样成立（`capacity_bytes` 由 ctx 决定）。
2. **recurrent RS buffer 随 parallel 线性增长：50.25 MiB/slot**（R 2.25 + S 48.0，32 层，F32 固定）——**hybrid fan-out 唯一随活跃分支数线性增长的显存项**。
3. **GPU 峰值完全在 8GB 预算内**：p8 q8_0 = 3290 MiB（8188 的 40%），余量约 4.9 GiB。
...
```
- 决策点 prompt 采用枚举式约束（"只能输出上述 ACTION 之一，不得输出其他内容"），规避 0.8B/4B 自由文本指令漂移；
- `temperature=0, seed=42` 固定 → 分支选择在**给定公共上下文下确定性**；用 0.8B（CPU）离线校准决策点 prompt 的合法输出率，4B（GPU）正式；
- 分支语义稳定：分支 prompt = 公共上下文 + 决策点输出 + 分支专属任务 + 分支专属 canary；分支任务**不依赖模型继续决策**（否则 temp=0 下仍可能漂移），后续轮次为固定后续推理/工具调用。
...
[公共上下文+决策点] → 模型输出 ACTION: branch(bX)
  ...
  → 可选"选择/回收"：末轮模型输出 ACTION: select(bX)（或固定回收 erase）
```
...
- **temp=0 分支语义验证门禁**：决策点合法输出率 100%（≥20 次复现）、分支输出 canary 泄漏率 0、跨 run 分支归属一致（见 §8）。

### 3.2 模块边界（复用/新增，避免第二套约定）

- **复用**：`framework/workload.py`（Workload 抽象/注册表）、`framework/driver.py`、`framework/kv_probe.py`、`framework/sampler.py`、`metrics/`、`runner/runner.py` 落盘骨架、`e15_branch_concurrent.py` 的 server 生命周期（`start_server/stop_server`）与 gate_verdict 纯函数模式。
- **新增（下一实现阶段，本阶段不写）**：
  - `benchmark/workload/fanout.py` —— `@register FanoutWorkload`（name="fanout"）：决策点 prompt 构造、分支扩展、canary 注入、`run()`（决策请求 → barrier 并发分支 → 每分支 M 轮 → 回收）、`evaluate()`（task_success/隔离/回收/决策确定性）。
  - `benchmark/runner/m0_fanout_runner.py` —— CLI：`--server-bin --model --port --ctx-size --parallel --fanout {2,4,8} --prefix-len {short,medium,long} --branch-len {short,medium,long} --tool-rounds M --warmup --reps --out`；复用 e15_branch_concurrent 的 server 生命周期与门禁骨架；**运行模式沿用其 off/on 对照结构**（on = 当前 `--kv-prefix-share` 请求，4B 预期 capability-rejected，作为安全对照，§4）。
  - `benchmark/tests/test_fanout_workload.py`、`benchmark/tests/test_m0_fanout_runner.py` —— 纯函数（决策点解析/确定性/canary/gate/CLI 校验）+ mock server e2e。
- **不新增第二套 config/runner 约定**：fanout 参数走 CLI + `config.extra["fanout"]`（沿用现有 `BenchmarkConfig.extra`），注册为第五场景（`--scenario fanout`），结果落盘沿用 `results/bench_<ts>.json` 结构。
- **失败降级/回滚**：server 启动失败重试 N 次后跳过该配置并记 INVALID；`/metrics/kv` 不可用 → KVProbe 现有优雅降级；预算校验 fail-fast（同 e15_branch_concurrent 前置校验）；纯新增文件 + 默认 off → 删除文件即完整回滚。

### 3.3 请求序列与计时

每 replicate：source/决策点请求（slot 0）→ N 分支并发（slot 1..N，barrier）→ 每分支 M 轮 → `erase` 全部 → 断言回基线（`used_cells==0/active_sequences==0`）。warmup≥1、reps≥5（与 `benchmark/README.md:145` 约定一致），中位数报告。
...
| 对照 | 配置 | 目的 |
|---|---|---|
| **A. full-prefill baseline** | 无 `--kv-prefix-share`，每分支全量 prefill（`cache_prompt` 生效但无跨 slot 共享） | 分支内存/延迟基线（used_cells 峰值 = 各分支 used 之和的上界） |
...
矩阵维度：
- `ctx-size 4096`（与生产配置一致）；`parallel ∈ {2,4,8}`（≥ fan-out + 1 决策 slot + 1 spare）；
- `fan-out ∈ {2,4,8}`（`parallel >= fan-out+2` 前置校验）；
- prefix/branch 长度桶：`short`（~200 tok）/`medium`（~800 tok）/`long`（~2000 tok）——用 token 估算（2.5 chars/token，同 e15 惯例）；
- `warmup 2 / reps 5`，`temperature 0 / seed 42`；
- 端口：每 server 实例独立端口（默认 8080+i），server 生命周期 = 每配置矩阵单元启动/停止（同 e15_branch_concurrent 模式）；
- **RTX 4060 8GB 安全预算（§1.3 实测）**：p8 q8_0 峰值 3290 MiB（40%）；矩阵全部组合峰值 < 4.0 GiB，余量 >4 GiB 供 compute buffer 与抖动——**无需降级**。

---

## 5. 落盘 schema 与硬门禁

### 5.1 指标字段（沿用现有行结构扩展，不新建第二套）

| 类别 | 字段 | 来源 |
|---|---|---|
| GPU/RSS 峰值 | `peak_gpu_mb` / `peak_rss_mb` | `sampler.find_server_gpu_mb/rss_mb`（行级 max，`metrics.py:104-105`） |
...
| token 分解 | `timings.prompt_n/cache_n/predicted_n`（`prompt_n+cache_n==prompt_tokens`） | `/completion` timings（`server-context.cpp:568-576`） |
| 延迟 | `latency_ms`、TTFT（首 token 时间，若需要则取 timings） | driver 行 / timings |
| 输出 | `tokens`（数组）+ `content_sha256` | driver/runner 提取（e15 模式） |
| 分支质量 | `task_success`、canary 泄漏标志、决策点合法率、回收后 `used_cells==0/active_sequences==0` | workload `evaluate` + gate |
...
- M0 归因方案：**启动日志解析 `RS buffer size`/`R/S (f32)`（`llama-memory-recurrent.cpp:123-126`）+ nvidia-smi 峰值差分**（全模型峰值 − 空载峰值 = KV+RS+compute），即可分离 attention KV（固定 68/128 MiB）与 recurrent（50.25 MiB/slot）。后续若需运行期 recurrent 计数，需 llama.cpp 最小扩展（`llama_kv_stats` 增加 recurrent 字段或 `/metrics/kv` 加 breakdown）——**列为未决问题 §9，不在 M0 承诺**。

### 5.2 指标语义区分

- **"预分配峰值不变"**：`capacity_bytes`/`capacity_cells` 由 ctx 决定（§1.3 实测 68 MiB@ctx4096），分支数不改变 capacity —— 这是预期，不是收益；
- **"used cells/容量改善"**：`used_cells` 随分支活跃/回收变化；改善指标 = 同 capacity 下承载更多活跃分支（used_cells 峰值占比）或回收后回落（`active_sequences==0`）；
- **recurrent 是唯一随分支线性增长的显存项**（50.25 MiB/slot）→ 分支容量上限由 `(GPU 余量 − 固定 KV/权重) / 50.25` 决定，量化验收用此式与实测对账。
...
| 文件 | 类型 | 内容 |
|---|---|---|
| `benchmark/workload/fanout.py` | 新增 | `FanoutWorkload`（name="fanout"）：决策点 prompt/分支扩展/canary/run/evaluate；`params_from_config` 支持 `config.extra["fanout"]` |
| `benchmark/runner/m0_fanout_runner.py` | 新增 | CLI + server 生命周期（复用 e15 骨架）+ 矩阵编排 + off/on 对照 + gate + JSON 落盘（`results/bench_m0_<ts>.json`） |
| `benchmark/tests/test_fanout_workload.py` / `test_m0_fanout_runner.py` | 新增 | 纯函数（决策解析确定性/canary/gate/CLI）+ mock server e2e |
| `benchmark/workload/__init__.py` | 修改 | 注册第五场景 import |
| 文档 | 更新 | 本文件补实测结果 + AGENTS.md 状态更新 |

CLI 合同：`--fanout {2,4,8} --prefix-len {short,medium,long} --branch-len {short,medium,long} --tool-rounds M --warmup 2 --reps 5 --out <path>`（其余沿用 e15_branch_concurrent：`--server-bin --model --port --ctx-size --parallel --cache-ram --min-lcp`）。配置合同：`config.extra["fanout"] = {"branches": n, "prefix_len": "...", "branch_len": "...", "tool_rounds": m}`，`--scenario fanout` 触发。

失败降级：启动失败重试 3 次 → 跳过配置记 INVALID；KVProbe 不可用降级（现有）；budget fail-fast。回滚：删除新增文件 + 移除注册行即完全回滚（默认 off，无行为改动）。

测试计划：纯函数 pytest（决策点解析边界/确定性/canary 注入与检测/gate_verdict 分支/CLI 校验/预算 fail-fast）+ mock OpenAI server e2e（现有 `tests/mock_server.py` 模式）+ 可选 TinyLlama CPU smoke（**仅冒烟，不宣称 4B 结论**）。

---

## 7. 8GB 可行性（探针实证，§1.3）

- 4B Q4_K_M + ctx4096 + unified + parallel 8 + q8_0：**GPU 峰值 3290 MiB / 8188（40%）**，全部组合 < 4.0 GiB；
- 权重 ~2.8 GiB 固定；attention KV 固定 68（q8）/128（f16）MiB；recurrent 50.25 MiB/slot 线性；
- 分支容量上限估算：`(8188 − ~2900 固定) / 50.25 ≈ 105 slot`（理论），但 decode 吞吐/批处理预算先于显存成为限制 → 矩阵取 2/4/8 是安全的。
- **结论：矩阵无显存阻塞；无需降级配置**。
...
**建议：GO** —— 调研充分（全部关键接口有 file:line 证据）、可行性实证（§1.3 探针）、无阻塞；M0 workload 是纯新增、可回滚、复用现有框架，风险可控。

可量化验收标准（M0 实现阶段）：
| 门禁 | 标准 |
|---|---|
| G-M0-1 决策确定性 | temp=0/seed=42 下 4B 决策点合法 `ACTION: branch(bX)` 输出率 **100%**（≥20 次，跨 2 次独立 server 会话） |
| G-M0-2 隔离 | canary 跨分支泄漏率 **0**（全部 reps×branches 输出） |
| G-M0-3 回收 | 每 rep 末 `erase` 后 `/metrics/kv` `used_cells==0 && active_sequences==0`（100%） |
| G-M0-4 内存归因 | 各成分可分离：`attention KV`（固定 68/128 MiB@ctx4096）、`recurrent`（50.25 MiB/slot）、`权重+compute`（差分）—— 与探针数字对账误差 ≤5% |
| G-M0-5 对照有效性 | 4B 上 off vs `--kv-prefix-share`：两者 `shared_cells` 恒 0，且 on 会话日志含 `E8-C1: capability rejected: hybrid`（100%） |
| G-M0-6 复现 | 同配置两次独立 run：TTFT/latency 中位数偏差 ≤10%，决策分支归属一致 |
| G-M0-7 数据落盘 | 每 rep 完整记录 §5.1 字段，无缺键；`results/bench_m0_*.json` 可独立复算 summarize |
...
2. **4B 决策点提示工程**：需 0.8B 离线校准（CPU，可跑）→ 4B 验证；若枚举约束下 4B 仍不稳定，fallback = 固定决策映射（标注非真实决策）。
3. **temp=0 下分支"选择/回收"轮语义**：模型 `ACTION: select(bX)` 的稳定性需实测；失败则降级为固定回收（`erase`），不阻塞内存归因目标。
4. **parallel 8 + long 桶的 decode 吞吐**：单线程调度下 latency 可能高，M0 只报告、不优化。
5. **build-cuda 二进制版本**：探针用 f54930492（未重建）；正式 M0 实验前需 `cmake --build build-cuda` 重建到 4a699aaad（含 afbf375c6）。

## 10. 风险

- Laptop GPU 抖动（E3 门禁先例）→ warmup + 中位数 + 同硬件 off/on paired。
- 0.8B 指令遵循不稳定 → 决策点枚举约束 + 离线校准（§9.2 fallback）。
- 模型可用性：4B GGUF 已就位（2.7 GB，`models/qwen3-5-4B-Q4_K_M.gguf`）；0.8B 需 `download_models.sh 0.8b`（M0 实现时按需拉取）。

## 11. 实施清单（下一步，不在本阶段执行）

1. 新增 `benchmark/workload/fanout.py` + 注册 + 纯函数测试（决策点解析/确定性/canary/回收断言）；
2. 新增 `benchmark/runner/m0_fanout_runner.py`（复用 e15 生命周期/gate 骨架）+ mock e2e 测试；
3. `cmake --build build-cuda` 重建 4B 实验二进制到 HEAD；
4. 0.8B 决策点校准 → 4B 决策确定性验证（G-M0-1）；
5. 跑矩阵（§4）→ 落盘 → 归因报告（§8 输出）→ 对照 G-M0-2..7；
