...
5. G-M0-1 专用稳定性验证：2 次**独立 server 启动** × 每次 ≥10 请求（总数 ≥20）
   ...
   —— **验证阶段失败路径（v12）**：
      ...
      b) 验证 server **启动失败 / 崩溃且 session 不完整 / decision_validation 证据序列化失败** → **phase=preflight 基础设施失败**（规则 `PREFLIGHT_INFRA` = `INVALID_RESULTS_SCHEMA_OR_INFRASTRUCTURE`）；**仅"崩溃且 session requests<10"走 5b（v25）；崩溃但完整按 5a**；
        **"证据无法落盘"两类区分（v27）**：
        a) **decision_validation 序列化/结构校验失败但结果 writer 可用** → 归因 `validation_incomplete`、`decision_validation=null`、**落盘 preflight 五键结果**（PREFLIGHT_INFRA）；
        b) **最终结果文件本身 I/O 写失败** → **无法承诺持久化**：stderr 输出固定错误码（`IO_WRITE_FAILED`）、**非零退出**，明确为**外部基础设施阻塞**，**不虚构结果文件**；
...
- **验证请求 retry 规则（v26）**：验证请求**同样使用 transient wrapper**；失败归因**三路（v26）**：① **`Popen.poll() != None` 确认进程已退出 → in-flight 请求立即归因 `server_crash`，不再 transient retry**；② **进程仍存活 → 连接/超时/5xx 按 0.25s/0.5s 重试，耗尽后归因 `connection_error`/`timeout`/`http_5xx`**；③ **首次异常时进程存活、但重试期间进程退出 → 归因 `server_crash`**（不再继续重试）；测试⑩ 补三路用例；
