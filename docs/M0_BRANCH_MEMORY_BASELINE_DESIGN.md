...
**v69（两轮完整 24-unit 4B formal 矩阵 run6/run7——wrapper 启用后的正式结论，2026-08-09）**：（1）**run6/run7 均 `verdict=PASS`**：120/120 reps OK、0 ERROR（vs run5 36/120 connection_error——transient retry 是 run5 高失败率根因修复，非掩盖）、24/24 units、12 groups、matrix_complete=true、canonical validator OK、notes=0；gates G-M0-1/2/3a/4/5/7 PASS、G-M0-3b 恒 N/A；（2）**G-M0-6 复现性跨轮判定 PASS**（单轮 gates N/A）：latency 中位数 467.0 vs 451.7 ms（dev 3.29%）、ttft 261.3 vs 267.0 ms（dev 2.15%）、decision_fallback 全 False 一致（阈值 ≤10%）；同 unit+rep 120 对 status 全一致、latency median +0.36%、peak_gpu/kv 跨轮 median 一致；（3）**严格 paired off/on**（同 run、同 fanout/bucket/ctk/ctv/rep、两边均 OK——120/120 OK → 每 run 60 对）：run6 all latency +0.24%/ttft -0.07%、run7 +0.62%/+0.99%——全维度 ±≤3%（除 run7 f8 ttft +10.27% n=10 小样本噪音）、两轮方向不一致 → 纯噪音、无系统性 on 收益；（4）**两层结论**：A) M0 benchmark/归因门禁 **PASS**（workload/生命周期/观测/归因/复现性完备）；B) on 机制 4B hybrid 被 capability gate 拒绝（key-lines `E8-C1: capability rejected: memory implementation does not support cross-slot prefix metadata sharing`）、shared_cells 恒 0 → **优化收益 NOT_APPLICABLE/NO_GAIN，benchmark PASS 不得称共享优化 PASS**；（5）资源峰值：组级 baseline gpu_used_mb 3026–3414（mean 3202，两轮逐 group 相同）、rep peak_gpu max 3424 MiB、kv peak cells max 3192/4096（无 OOM）；（6）归档 `benchmark/baseline/`：run6/run7 主 JSON + evidence + 15-tag key-lines + run6v7 paired JSON（`docs/M0_FINAL_RESULT.md` 清单）；run5 保持 INVALID_INTERMEDIATE；（7）无代码改动（纯运行+归档）——全量 pytest 沿用 v68 HEAD **758 passed**；llama.cpp 零改动。详细数据见 `docs/M0_FINAL_RESULT.md`。
