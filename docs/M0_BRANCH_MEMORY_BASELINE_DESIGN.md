...
> 修订：2026-08-09 v2（矩阵/预算/层数/架构/接口合同/回收观测）；v3（精确 token 计数、矩阵手算修正、driver 合同、门禁语义）；v4（driver `_retry` 位置参数兼容、thinking 字段生效实证、G-M0-1 与矩阵完全分离、RS 日志行号、CLI 临时目录）；v5（G-M0-1 口径统一、可提交证据、token parity 门禁、verdict 规则）；v6（INVALID 谓词 OR 语义、verdict/gates 值域映射、add_special 归因、parity 校准协议）；v7（gate/verdict 分层、parity 前置、schema 完整化、聚合定稿）；v8（preflight/schema 闭环）；v9（状态机/schema 终审）；v10（验证/聚合路径收口）；v11（跨章节同步定稿）；v12（运行边界合同）；v13（实现阻断收口）；**v14（validator/测试合同终审——测试③与 partial decision_validation 对齐（preflight 携带完整 formal 级 2 sessions 才非法，partial 合法且按已有结构校验，补正反单测）、formal validator 校验 error_count 类型与等式 requests=valid+invalid+error_count、权威测试清单扩至 ①–㉑（Driver 递归透传 ⑲、FORMAL_INCOMPLETE 优先于 REP_ERROR 显式单测 ⑳、parity_progress 单测 ㉑）、parity_progress 正式进入 meta schema（{completed: list[str], errors: list[去敏]}，不改变 PREFLIGHT_INFRA verdict）、planned_units=理论合法矩阵候选（6×2×2=24），rejections 仅记 tokenizer 复核后进一步被拒 unit、token_count_method 映射穷尽（formal/PARITY_MISMATCH/ALL_BUDGET_REJECTED→apply-template+tokenize；PREFLIGHT_INFRA→null；chars/2.0 仅 notes 诊断字段）、requests=0 → total_valid_rate=null、§6 测试计划以 §5.1 ①–㉑ 为唯一来源）**，见各节。
...
- **失败降级（v14 定稿，端点归因全文同步）**：`/apply-template` 或 `/tokenize` **探测失败、请求异常、端点消失或 schema 无法解析**（HTTP 非 200 / JSON 畸形 / 缺必需字段）→ **基础设施失败 → 落盘 preflight（规则 `PREFLIGHT_INFRA` = `INVALID_RESULTS_SCHEMA_OR_INFRASTRUCTURE`）并停止**；**仅当端点正常响应但 token parity 数值非零** → 落盘 preflight（规则 `PARITY_MISMATCH` = `HOLD_NOT_VALIDATED`）并停止；**预算超限同样落盘 preflight 五键结果（规则 `ALL_BUDGET_REJECTED` = `HOLD_NOT_VALIDATED`）——已取消 v8 的"不生成文件"行为**（保证证据不丢）；**`token_count_method` 映射穷尽（v14）**：formal / `PARITY_MISMATCH` / `ALL_BUDGET_REJECTED` → `"apply-template+tokenize"`；`PREFLIGHT_INFRA` → 统一 `null`（计数前或部分完成后端点消失，进度记入 `parity_progress`）；`chars/2.0` 仅作为 notes/meta **诊断字段的估算方法**（`notes["est_method"]="chars/2.0-diagnostic"`），**不作为正式 `token_count_method` 主值**，**不允许绕过 parity 开始正式矩阵**。
...
```json
{
  "meta": {"workload": "fanout", "design_ref": "M0_BRANCH_MEMORY_BASELINE_DESIGN.md",
           ...
           "decision_validation": {"sessions": [  # v13（M2/G-M0-1 证据）：两次独立 server 启动；formal 强制 2 sessions×≥10，preflight 允许 null/partial
               ...
               "total_valid_rate": 1.0},          # v14：requests>0 时 = valid/requests；requests=0 时必须 null；error_count>0 或 invalid>0 → G-M0-1=FAIL
...
- **phase 字段约束（v12，validator 校验必需字段与类型）**：
  ...
  - `phase=formal`：`decision_validation` **必须含两次独立 server 会话证据**（`sessions` 长度 = 2、每会话 `requests ≥ 10`、`valid/invalid/error_count` 为非负整数且**等式 `requests == valid + invalid + error_count` 成立（v14）**、`finish_reasons`/`output_hashes`/`error_summary` 类型正确；`total_valid_rate`：`requests>0` 时 `= valid/requests`，**`requests=0` 时必须 `null`（v14）**）；`preflight_rejections` 必须为数组（可为空）；`planned_units`/`executed_units` 为正整数且 `executed_units ≤ planned_units`；**`matrix_complete`（v12）必须为 bool，且 `matrix_complete=true` 时 `executed_units == planned_units − |preflight_rejections|`**；`parity_ok` 必须 `true`；`modes` 非空、`gates` 非空；
