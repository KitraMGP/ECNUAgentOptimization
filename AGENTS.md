...
- **M0（阶段 0 后首条路线）**：真实智能体多路径决策 workload + 分支内存基线——**调研完成 + 设计完成（DESIGN ONLY，v28 evidence failure 路径与测试合同收口），未实现 workload 代码、未运行正式 benchmark**；详见 `docs/M0_BRANCH_MEMORY_BASELINE_DESIGN.md`（实施建议 GO）。关键探针结论（4B Q4_K_M + ctx4096 + unified，RTX 4060 8GB）：**32 层 = 8 attention + 24 recurrent**（qwen35.cpp fallback interval=4）；attention KV 固定池 68 MiB(q8_0)/128 MiB(f16) 与分支数无关（per-cell q8 17408 B = 8×8×128×2×34/32）；recurrent state **50.25 MiB/slot** 随 parallel 线性增长（= 24 层 × (n_embd_r 24576 + n_embd_s 524288) × 4B，实测闭合）；parallel 10 q8_0 峰值 3402/8188 MiB 无显存阻塞；合法矩阵 = fanout{2,4,8}→parallel{4,6,10} × 桶 short(150/150)/medium(280/480)/long(400/600)，预算函数 `budget=P+N×(P+B) ≤ 3481 cells` 逐 unit（**P/B 由 /apply-template + /tokenize 实测**；一致性门禁 = chat `usage.prompt_tokens` vs tokenize 偏差 0，**证据归档 `benchmark/baseline/m0_probe_thinking_token_parity_20260809.json`**，add_special 幂等归因 = 该 GGUF `add_bos_token=false`、bos_token_id 缺失、add_eos_token 字段缺失（无 BOS），换模型必须重验）；`/metrics/kv` 只统计 attention（recurrent 无运行期接口；回收门禁仅 attention，GPU 回落仅 smoke）；架构定稿独立 runner（不注册第五场景）；driver 扩展保留 `_retry` 位置参数（driver.py:132 递归兼容、_extra_body :135-142）+ `sdk_max_retries` 可选参数（M0 实例化 `Driver(max_retry=1, sdk_max_retries=0)`，6-wire 上限；context400 递归透传参数）；thinking 合同：server 默认 thinking 开启，必须显式 enable_thinking=false（server-common.cpp:1095-1102 + 证据）；**决策谓词 → rep `status=INVALID_DECISION`（⇔ decision_fallback=true）；rep 状态机 OK/INVALID_DECISION/ERROR（ERROR 必填 error_type 固定 code；matrix_complete=true 时单 rep ERROR → REP_ERROR；matrix_complete=false → FORMAL_INCOMPLETE 优先）；G-M0-1 专用验证 <100% → G-M0-1=FAIL 但 formal 仍请求模型（仅谓词命中 rep fallback；INVALID_DECISION 两来源；等式与 finish_reasons 不变量均强制；output_hashes 长度 == valid+invalid；total_valid_rate=聚合；**验证崩溃/启动失败边界（v23-v28）：两次验证 session 均达 requests≥10 且每请求有记录 → 证据完整继续 formal、不要求 health 正常；5b 三触发权威定义（启动失败 / 崩溃且 session requests<10 / decision_validation 序列化失败），均立即停止验证、第一/第二 session 不完整都不启动 formal；首 session requests=0 无证据 → null、≥1 且 <10 → partial；证据路径（v28）：序列化异常 → 原子 null + validation_incomplete + 五键落盘 + exit 0（⑬b-d）；序列化成功但 validator 非法 → SCHEMA_INVALID + 保留 JSON 证据（⑬b-f）；最终文件 I/O 失败 → EXIT_IO_WRITE_FAILED=74（EX_IOERR）兜底 + 不虚构文件（⑬b-e）**；**验证请求 retry 三路（当前版本）：进程已退出 → 立即 server_crash；存活瞬时故障重试耗尽 → connection_error/timeout/http_5xx；首次异常存活但重试期间退出 → server_crash**；formal server 启动失败保留完整证据、modes/gates={}、planned_units=24/executed_units=0、matrix_complete=false、三类 code）；verdict 五值枚举 + 稳定规则名称 + 聚合签名；**planned_units 派生值：len(理论合法组合6)×len(cache_types2=ctk{q8_0,f16})×len(controls2=对照{A,B})=24；结构化错误码（11 枚举；error_summary 无错误 []/有错误 {code,stage?,bucket?,count}（sum==error_count、唯一性键 (code, stage ?? null, bucket ?? null)、count 正整数）；preflight_reason 规则映射；token_count_method 彻底唯一；chars/2.0 正式诊断、chars/2.5 仅旧估算对照；ACTION 统一 branch(bX)；测试①–㉖（⑬a-f 六子例、⑰ 清理固定序列（含 IO_WRITE_FAILED 兜底仍 finally 清理）、㉖ 5b rejections 正反）**；M0 不用 OpenAI tools schema；探针二进制 8569/afbf375c6（核心代码与 HEAD 等价）。
