...
- **M0（阶段 0 后首条路线）**：真实智能体多路径决策 workload + 分支内存基线——**调研完成 + 设计完成（DESIGN ONLY），未实现 workload 代码、未运行正式 benchmark**；详见 `docs/M0_BRANCH_MEMORY_BASELINE_DESIGN.md`（实施建议 GO）。关键探针结论（4B Q4_K_M + ctx4096 + unified，RTX 4060 8GB）：attention KV 固定池 68 MiB(q8_0)/128 MiB(f16) 与分支数无关；recurrent state **50.25 MiB/slot** 随 parallel 线性增长（hybrid fan-out 唯一随分支线性增长的显存项）；p8 q8_0 峰值 3290/8188 MiB 无显存阻塞；`/metrics/kv` 只统计 attention 部分（recurrent 无运行期接口，观测缺口已记录）。
