...
- **M0（阶段 0 后首条路线）**：真实智能体多路径决策 workload + 分支内存基线——**调研完成 + 设计完成（DESIGN ONLY，v29 SCHEMA_INVALID 落盘合同收口），未实现 workload 代码、未运行正式 benchmark**；详见 `docs/M0_BRANCH_MEMORY_BASELINE_DESIGN.md`（实施建议 GO）。关键探针结论（4B Q4_K_M + ctx4096 + unified，RTX 4060 8GB）：**32 层 = 8 attention + 24 recurrent**（qwen35.cpp fallback interval=4）；attention KV 固定池 68 MiB(q8_0)/128 MiB(f16) 与分支数无关（per-cell q8 17408 B = 8×8×128×2×34/32）；recurrent state **50.25 MiB/slot** 随 parallel 线性增长（= 24 层 × (n_embd_r 24576 + n_embd_s 524288) × 4B，实测闭合）；parallel 10 q8_0 峰值 3402/8188 MiB 无显存阻塞；合法矩阵 = fanout{2,4,8}→parallel{4,6,10} × 桶 short(150/150)/medium(280/480)/long(400/600)，预算函数 `budget=P+N×(P+B) ≤ 3481 cells` 逐 unit（**P/B 由 /apply-template + /tokenize 实测**；一致性门禁 = chat `usage.prompt_tokens` vs tokenize 偏差 0，**证据归档 `benchmark/baseline/m0_probe_thinking_token_parity_20260809.json`**，add_special 幂等归因 = 该 GGUF `add_bos_token=false`、bos_token_id 缺失、add_eos_token 字段缺失（无 BOS），换模型必须重验）；`/metrics/kv` 只统计 attention（recurrent 无运行期接口；回收门禁仅 attention，GPU 回落仅 smoke）；架构定稿独立 runner（不注册第五场景）；driver 扩展保留 `_retry` 位置参数（driver.py:132 递归兼容、_extra_body :135-142）+ `sdk_max_retries` 可选参数（M0 实例化 `Driver(max_retry=1, sdk_max_retries=0)`，6-wire 上限；context400 递归透传参数）；thinking 合同：server 默认 thinking 开启，必须显式 enable_thinking=false（server-common.cpp:1095-1102 + 证据）；**决策谓词 → rep `status=INVALID_DECISION`（⇔ decision_fallback=true）；rep 状态机 OK/INVALID_DECISION/ERROR（ERROR 必填 error_type 固定 code；matrix_complete=true 时单 rep ERROR → REP_ERROR；matrix_complete=false → FORMAL_INCOMPLETE 优先）；G-M0-1 专用验证 <100% → G-M0-1=FAIL 但 formal 仍请求模型（仅谓词命中 rep fallback；INVALID_DECISION 两来源；等式与 finish_reasons 不变量均强制；output_hashes 长度 == valid+invalid；total_valid_rate=聚合；**验证崩溃/启动失败边界（v23-v29）：两次验证 session 均达 requests≥10 且每请求有记录 → 证据完整继续 formal、不要求 health 正常；5b 三触发（启动失败 / 崩溃且 session requests<10 / decision_validation 序列化失败），崩溃仅不完整走 5b、启动/序列化独立触发；序列化失败无论 session 多少原子 null 不启动 formal；⑬b-c requests=0 → null 边界；**SCHEMA_INVALID 落盘合同（v29）：12 error code（新增 schema_invalid）、主结果 preflight 五键 envelope（preflight_reason=schema_invalid、verdict=INVALID_RESULTS_SCHEMA_OR_INFRASTRUCTURE、planned_units=24/executed_units=0/matrix_complete=false）、诊断 sidecar `results/m0_fanout_<ts>.schema_invalid.json`（{path, code, expected_type, actual_type}，不存 prompt/URL/路径值/自由文本，finally 不删）、固定退出码（0 合法落盘 / 65 EXIT_SCHEMA_INVALID / 74 EXIT_IO_WRITE_FAILED 升级）、序列化失败落盘失败 → 74 组合测试**；**验证请求 retry 三路（当前版本）**；formal server 启动失败保留完整证据、modes/gates={}、三类 code）；verdict 五值枚举 + 稳定规则名称 + 聚合签名；**planned_units 派生值：len(理论合法组合6)×len(cache_types2=ctk{q8_0,f16})×len(controls2=对照{A,B})=24；结构化错误码（12 枚举；error_summary 无错误 []/有错误 {code,stage?,bucket?,count}（sum==error_count、唯一性键 (code, stage ?? null, bucket ?? null)、count 正整数）；preflight_reason 规则映射；token_count_method 彻底唯一；chars/2.0 正式诊断、chars/2.5 仅旧估算对照；ACTION 统一 branch(bX)；测试①–㉖（⑬a-f 六子例组级说明移组首、⑰ 清理固定序列（results 主文件/sidecar 不清理）、㉖ 5b rejections 正反）**；M0 不用 OpenAI tools schema；探针二进制 8569/afbf375c6（核心代码与 HEAD 等价）。
